...
UPDATE_INTERVAL = 10      # Reducido a 10 segundos
MESSAGE_COOLDOWN = 30     # Aumentado a 30 segundos para evitar duplicados
REQUEST_TIMEOUT = 30      # Timeout para peticiones HTTP
DUPLICATE_WINDOW = 120    # Ventana de tiempo para detectar duplicados (2 minutos)
...
class MatchTracker:
    ...
    def get_live_matches(self):
        try:
            ...
            logger.error(f"Error inesperado en get_live_matches: {str(e)}")
            return []
    ...
    def get_match_details(self, match_id):
        try:
            ...
            logger.error(f"Error al obtener detalles del partido: {str(e)}")
            return None
        except Exception as e:
            logger.error(f"Error inesperado en get_match_details: {str(e)}")
            return None
    ...
    def send_message(self, message, message_type, match_id):
        ...
        if not self.is_duplicate_message(message_key, message):
            try:
                asyncio.get_event_loop().run_in_executor(self.executor, self.telegram_channel.send_message, message)
        ...
        try:
            ...
            if match_id not in self.tracked_matches:
                ...
                if current_status in ['LIVE', 'IN_PLAY']:
                    ...
                    self.send_message(message, 'start', match_id)
                ...
                if (previous_status == 'PAUSED' and 
                    ...
                    self.send_message(message, 'second_half', match_id)
                    ...
                    if match_details and 'goals' in match_details:
                        for goal in match_details['goals']:
                            ...
                            if goal_id not in tracked_match['goals']:
                                ...
                                self.send_message(message, f'goal_{goal_id}', match_id)
                ...
                if current_status == 'PAUSED' and not tracked_match['half_time_notified']:
                    ...
                    self.send_message(message, 'half_time', match_id)
                ...
                if (current_status in ['FINISHED', 'COMPLETED'] and 
                    ...
                    self.send_message(message, 'end', match_id)
...
async def main():
    ...
    try:
        ...
        if tracker:
          tracker.send_message("⚠️ *Bot de Fútbol Reiniciando debido a un error* ⚠️", 'bot_error', 0)
    finally:
        if tracker and tracker.executor:
          tracker.executor.shutdown(wait=True)
